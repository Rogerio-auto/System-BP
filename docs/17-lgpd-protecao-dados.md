...
| 3   | Histórico persistente do copiloto interno (referência + hidratação viva) | **Em elaboração (Fase 0)** | Antes da Fase 1 (código)        | DPO técnico (Rogério Viana) | DPIA em `docs/anexos/lgpd/dpia-historico-copiloto.md`. Base: Art. 7º IX. **Sem novo suboperador** (hidratação local) e **sem PII de cliente em repouso** (só referências + narrativa sem PII). Código bloqueado até parecer do DPO oficial.   |

...
| 9   | Histórico do copiloto interno                           | Art. 7º IX (legítimo interesse — produtividade operacional), ancorado na base do atendimento subjacente                                                                                                                  | **Sem PII de cliente em repouso**: pergunta higienizada (nome/CPF/telefone mascarados) + narrativa sem PII + referências de entidade (`lead_id`); dado sensível hidratado ao vivo, nunca gravado                                                                                                                                                                                                                                                                                                                         | Operador interno (uso do copiloto)                                       | 90 dias (job de purga)                                                                                                                                                                                                    | Não                                                                                           | — (hidratação local)                                                                                                                                                               |
| 10  | Escalação de lead ao Crédito (notificação interna)      | Art. 7º IX (legítimo interesse — comunicação interna para gestão da política pública), ancorado na base do atendimento subjacente                                                                                        | Referência ao lead (`lead_id`) + nota livre do operador — **sem PII bruta no outbox/evento** (`assistant.escalation.created` não carrega CPF/telefone/nome bruto de cidadão; destinatário hidrata pelo próprio escopo)                                                                                                                                                                                                                                                                                                   | Operador interno (ação humana confirmada, doc 22 §12.8)                  | Conforme política já vigente de notificações (job `cron-retention`, `docs/23-notificacoes.md` §9)                                                                                                                         | Não                                                                                           | —                                                                                                                                                                                  |
| 11  | Notificação operacional interna via Web Push (PWA)      | Art. 7º IX (legítimo interesse do controlador — entregar ao operador o alerta de SLA/handoff/evento com o app fechado) + Art. 7º V (execução — operação da política pública), ancorado na base do atendimento subjacente | Subscription de push do device do colaborador: `endpoint` (URL do push service), `p256dh` e `auth` (chaves ECDH do client), `user_agent` (rótulo do device para gestão) — tabela `push_subscriptions`. **Payload do push sem PII** (só `title` genérico + `severity` + `entity_type`/`entity_id` para deep-link; doc 24 §5.3)                                                                                                                                                                                            | Colaborador interno (opt-in explícito num gesto do usuário, doc 24 §5.4) | Enquanto a subscription estiver ativa: soft-delete no opt-out/logout + remoção automática de subscription morta (respostas `404`/`410` do push service, doc 24 §5.2/§9); job de retenção de órfãs previsto como follow-up | Não (interno)                                                                                 | **Push services do navegador** (FCM/Google, Mozilla, Apple, Microsoft WNS) — canal de terceiro **não-confiável**: recebem apenas o payload cifrado sem PII, nunca conteúdo pessoal |
| 12  | Biblioteca de respostas rápidas do live chat (Fase F28) | Art. 7º IX (legítimo interesse — produtividade operacional do atendimento), ancorado na base legal da finalidade 1 (o disparo em si é atendimento ao cidadão)                                                            | **Dado de colaborador** (`owner_user_id`/`created_by` — autoria/propriedade do atalho) + **conteúdo institucional** (título/corpo/categoria/mídia, redigido pela gestão ou pelo próprio operador). O corpo é validado contra CPF/CNPJ/e-mail/telefone (doc 17 §8.4) antes de salvar — não deve conter PII do cidadão. **Nome do cidadão (`{{contato.*}}`) nunca é persistido**: a interpolação é 100% client-side, em memória, só no instante do uso (doc 25 §6.2) — a tabela `quick_replies` é isenta de PII do titular | Operador/gestor interno (cadastro via tela admin)                        | Enquanto a resposta rápida existir (soft-delete manual pela gestão) — acervo institucional, sem retenção temporal própria por não ser dado de conversa                                                                    | Não                                                                                           | — (mídia hospedada no próprio storage da organização, prefixo `quick-replies/{orgId}/`)                                                                                            |
...
> **Transferência internacional (finalidade 11 — Web Push).** Os push services (FCM/Google, Mozilla, Apple, Microsoft WNS) são operados no exterior: o `endpoint` da subscription trafega por infra estrangeira. Registra-se essa característica de transferência internacional. O risco é mitigado por design — o payload **não carrega PII** (doc 24 §5.3), tratado como canal não-confiável — e o titular é colaborador interno (não cidadão), mas a natureza internacional do subprocessamento fica documentada aqui.
...
| `payment_dues` (boleto)              | `boleto_url`, `boleto_media_id`, `boleto_digitable_line`, `pix_copia_cola`, `boleto_filename`      | Financeiro + documento com PII                                                             | Boleto contém nome/CPF/endereço. **Não armazenamos os bytes do PDF** — só referência. `boleto_url` deve ser controlada/assinada (allowlist de host, F5-S13); nunca URL pública permanente. Redact §8.3; outbox nunca carrega esses campos; retenção = 5 anos (igual à parcela)                                                                                                                                                                                                                                                                               |
...
| `push_subscriptions`                 | `endpoint`, `p256dh`, `auth`, `user_agent`                                                         | Técnico — identificador de device                                                          | `endpoint`/`p256dh`/`auth` identificam o device do colaborador (dado pessoal). Cobertos por `pino.redact` (`endpoint`, `keys`, `p256dh`, `auth` → `[REDACTED]`; §8.3, implementado em F27-S06). Payload de push **sem PII** (doc 24 §5.3). Soft-delete no opt-out/logout + remoção em `404`/`410`; só do próprio usuário autenticado (sem cross-user)                                                                                                                                                                                                        |
| `quick_replies` (Fase F28)           | `owner_user_id`, `created_by`                                                                      | Dado de colaborador (identificador funcional) — **não é tabela de PII do titular/cidadão** | `body`/`title`/`category` são conteúdo institucional redigido por humano; validação de cadastro rejeita CPF/CNPJ/e-mail/telefone do cidadão no corpo (doc 25 §12, reusa os padrões de §8.4 deste doc). `{{contato.*}}` interpolado **só no cliente**, nunca gravado. `audit_logs` de `quick_reply.created/updated/deleted` nunca incluem `body` — só `quickReplyId`/`shortcut`/`visibility`. Mídia em prefixo próprio do bucket (`quick-replies/{organizationId}/`), fora da rotina de retenção de atendimento (é ativo institucional, não dado de conversa) |
...
| Histórico do copiloto interno (esqueleto + refs, sem PII)       | 90 dias                                                        | Job diário                          | Art. 16 (eliminação após fim da finalidade) |
...
| 3   | Histórico persistente do copiloto interno (referência + hidratação viva) | **Em elaboração (Fase 0)**                                              | Antes das Fases 2–4 (persistência)                          | DPO técnico (Rogério Viana) | DPIA em `docs/anexos/lgpd/dpia-historico-copiloto.md`. Base: Art. 7º IX. **Sem novo suboperador** (hidratação local) e **sem PII de cliente em repouso** (só referências + narrativa sem PII). Portão = persistência: Fases 2–4 bloqueadas até parecer do DPO oficial; Fase 1 (refactor sem persistência) liberada.                                                                                                                                                                                                                                                               |
| 4   | Web Push / notificação operacional interna (finalidade #11, F27-S06)     | **Conclusão técnica: não dispara DPIA autônoma — decisão final do DPO** | Aval do DPO antes de ligar a flag `pwa.enabled` em produção | DPO técnico (Rogério Viana) | Leitura preliminar (doc 24 §9): **não há novo risco alto**. Reutiliza padrão já avaliado (notificações internas F24), **payload sem PII** (doc 24 §5.3), titular é **colaborador interno** (não cidadão vulnerável), sem decisão automatizada. O único fluxo novo é a subscription (`endpoint`/`p256dh`/`auth`) por push services estrangeiros — registrado como transferência internacional (§3.3 nota, §12) e mitigado por design. Aciona o gatilho §11.1 "novo suboperador", por isso fica registrado aqui; **a conclusão de dispensa é do DPO oficial**, não do time técnico. |
...
| **Push services do navegador** (FCM/Google, Mozilla, Apple, Microsoft WNS) | Entrega de Web Push interno (finalidade #11)        | EUA / variável (exterior) | `endpoint` da subscription + payload **cifrado sem PII** (só `title` genérico + deep-link) | Termos do respectivo provedor de navegador      | Canal de terceiro **não-confiável** (doc 24 §5.3): nunca recebe nome/telefone/CPF/valor/mensagem. Roteado pelo navegador do próprio colaborador (opt-in). Sem DPA dedicado — mitigação por ausência de PII no payload. Adicionado em F27-S09 — 2026-07-20. |
...
| 1.2    | 2026-07-20 | Backend Engineer (F27-S09)              | RoPA do Web Push (F27-S06): §3.3 finalidade #11 (notificação operacional interna via Web Push, `push_subscriptions`, payload sem PII) + nota de transferência internacional; §3.4 linha `push_subscriptions` com redact; §12.1 push services do navegador como suboperadores estrangeiros não-confiáveis; §11.3 DPIA #4 (conclusão técnica: sem novo risco alto — aval final do DPO). |

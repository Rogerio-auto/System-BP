...
- Base legal documentada por finalidade (atendimento, simulação, oferta).
- Consentimento via WhatsApp registrado em `customers.consent_at` quando aplicável.
- Direitos do titular:
  - Acesso: relatório consolidado por `customer_id`.
  - Correção: via tela de cliente.
  - Apagamento: rotina de pseudonimização (mantém audit, remove PII).
  - Portabilidade: export JSON.
- DPO/responsável e contato cadastrados no Manager (`organizations.settings.dpo`).

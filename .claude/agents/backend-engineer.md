...
> Rode `git add` + `git commit` do seu código de implementação (`feat(...): ... (<SLOT-ID>)`)
> **antes** do `finish`. Depois do push, confirme com `git log --stat origin/feat/<slot-id>`
> que seus arquivos estão no log — se só houver commits `chore(tasks)`, o código não subiu.

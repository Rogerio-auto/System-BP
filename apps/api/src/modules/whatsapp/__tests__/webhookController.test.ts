// =============================================================================
// __tests__/webhookController.test.ts — Testes do handler template_status_update.
//
// Contexto: F5-S09 + security fixes M-1.
//
// Estratégia:
...
//   - M-1: confirma que payload message_template_status_update chega em processTemplateStatusUpdates
//     (via hasTemplateStatusUpdates gate + processTemplateStatusUpdates call — replica a lógica de routes.ts).
// =============================================================================
...
// ---------------------------------------------------------------------------
// Fix M-1: wiring test — replica a lógica de routes.ts
//
// Confirma que um payload com field=message_template_status_update:
//   1. É detectado por hasTemplateStatusUpdates() → true
//   2. Chega em processTemplateStatusUpdates() e é processado
//
// Isso espelha a sequência do handler POST /api/whatsapp/webhook após o rebase:
//   if (hasTemplateStatusUpdates(parsedBody)) {
//     await processTemplateStatusUpdates(validPayload, orgId, correlationId);
//   }
// ---------------------------------------------------------------------------

describe('M-1 wiring: payload message_template_status_update chega em processTemplateStatusUpdates', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockTransaction.mockImplementation(async (fn: (tx: unknown) => Promise<void>) => {
      await fn({});
  ...
  it('payload message_template_status_update é detectado e processado', async () => {
    const rawPayload = {
      object: 'whatsapp_business_account',
      entry: [
        {
    ...
    // Passo 1: detectar (como routes.ts faz)
    expect(hasTemplateStatusUpdates(rawPayload)).toBe(true);

    // Passo 2: processar (mock de resultado positivo)
    mockUpdateByMetaId.mockResolvedValue({
      id: 'tmpl-99',
      status: 'approved',
      organizationId: ORG_ID,
    });
    ...
    // Passo 3: parsear e chamar (como routes.ts faz após TemplateStatusWebhookPayloadSchema.safeParse)
    const { TemplateStatusWebhookPayloadSchema } = await import('../webhookController.js');
    const parseResult = TemplateStatusWebhookPayloadSchema.safeParse(rawPayload);
    expect(parseResult.success).toBe(true);
    if (!parseResult.success) return;

    const result = await processTemplateStatusUpdates(parseResult.data, ORG_ID, CORRELATION_ID);

    // O payload chegou em processTemplateStatusUpdates e foi processado
    expect(result.processed).toBe(1);
    expect(result.skipped).toBe(0);
    expect(mockUpdateByMetaId).toHaveBeenCalledWith(expect.anything(), '99999', ORG_ID, 'approved');
  });

  it('payload messages (não template_status_update) não dispara processTemplateStatusUpdates', () => {
    ...
    // hasTemplateStatusUpdates deve retornar false → routes.ts não chama processTemplateStatusUpdates
    expect(hasTemplateStatusUpdates(messagesPayload)).toBe(false);
    // Confirma que mockUpdateByMetaId não foi chamado (não houve dispatch)
    expect(mockUpdateByMetaId).not.toHaveBeenCalled();
  });
});

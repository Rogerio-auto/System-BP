...
describe('POST /internal/cities/identify', () => {
  ...
  it('retorna matched: false com alternatives quando confidence < 0.85', async () => {
    mockFindCitiesByFuzzyMatch.mockResolvedValueOnce([
      makeCandidate({ id: FIXTURE_CITY_ID,   name: 'Porto Velho', similarity: 0.70 }),
      makeCandidate({ id: FIXTURE_CITY_ID_2, name: 'Ji-Paraná',   similarity: 0.60 }),
      makeCandidate({ id: FIXTURE_CITY_ID_3, name: 'Vilhena',     similarity: 0.50 }),
    ]);
    ...
    expect(body.alternatives[0]).toMatchObject({ city_id: FIXTURE_CITY_ID, city_name: 'Porto Velho' });

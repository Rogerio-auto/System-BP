...
function setupMockSequence(opts: {
  ...
  const cityResult = 'city' in opts
    ? (opts.city !== null ? opts.city : null)
    : { name: 'Porto Velho' };
  const agentResult = 'agent' in opts
    ? (opts.agent !== null ? opts.agent : null)
    : { displayName: 'Agente Teste' };
  const kanbanResult = 'kanban' in opts
    ? (opts.kanban !== null ? opts.kanban : null)
    : { stageName: 'Qualificação' };
  const simulationResult = 'simulation' in opts
    ? (opts.simulation !== null ? opts.simulation : null)
    : makeSimulationRow();

...
class Settings(BaseSettings):
    ...
    # Slug OpenRouter válido. `anthropic/claude-3.5-haiku` foi descontinuado no
    # OpenRouter (404 "No endpoints found") e quebrava o classify_intent em prod
    # (LLMProviderError → handoff forçado). Corrigido em 2026-06-28.
    model_classifier: str = Field(
        default="anthropic/claude-haiku-4.5", validation_alias="LLM_MODEL_CLASSIFIER"
    )
    # Kimi K2 (moonshotai/kimi-k2) é o reasoner default: throughput alto, custo competitivo
    ...
    # NB: namespace correto no OpenRouter é `moonshotai/` (não `moonshot/`).
    model_reasoner: str = Field(
        default="moonshotai/kimi-k2", validation_alias="LLM_MODEL_REASONER"
    )
